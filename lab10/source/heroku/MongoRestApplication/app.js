...
angular.module('myApp', [])
    ...
    .controller('View1Ctrl', function ($scope, $http) {
                ...
                handler.success(function (data) {
                    ...
                    if (data != null && data.response != null && data.response.venues != undefined && data.response.venues != null) {
                        for (var i = 0; i < data.response.venues.length; i++) {
                            ...
                            var req = $http.post('http://127.0.0.1:8081/register',$scope.venueList[i]);
        ...
        $scope.finddata = function(db, callback) {
            var cursor =db.collection('table1').find( );
            cursor.each(function(err, doc) {
                assert.equal(err, null);
                if (data != null && data.response != null && data.response.datalist != undefined && data.response.datalist != null) {
                    for (var i = 0; i < data.response.datalist.length; i++) {
                        $scope.venueList[i] = {
                            "name": data.response.datalist[i].name,
                            "id": data.response.datalist[i].id,
                            "location": data.response.datalist[i].location
                        };
                        var config = {
                            headers : {
                                'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'
        ...
        $scope.getactor = function () {
            ...
            var params = {
                ...
                'limit': 10,
                'indent': true,
                'key' : 'AIzaSyAXnHf8d9Ia3a51RGmvnefr1kYrUYc2H74',
            };
